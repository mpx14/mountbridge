"""GTK3 widget classes: dialogs, cards, discovery panel.

Cards are views only: they render state and forward clicks to callbacks.
MainWindow owns all mount operations and threads.
"""
import os
from pathlib import Path
from typing import Callable, Optional

import gi

gi.require_version("Gtk", "3.0")
from gi.repository import Gtk, Pango

from .constants import MOUNTS_DIR, SYS_MOUNT_BASE
from .discovery import Discovery
from .models import LiveMount, MountConfig, MountType
from .parsing import slugify, valid_host

TYPES = ["nfs", "smb", "sshfs"]


def _css(widget, *classes):
    ctx = widget.get_style_context()
    for c in classes:
        ctx.add_class(c)
    return widget


# ── Add / Edit mount dialog ──────────────────────────────────────────────────

class MountDialog(Gtk.Dialog):
    def __init__(self, parent, mount: Optional[MountConfig] = None, prefill=None):
        super().__init__(transient_for=parent, modal=True, use_header_bar=True)
        self.set_title("Edit Mount" if mount else "Add Mount")
        self.set_default_size(500, -1)
        self.edit = mount
        self._build(mount or prefill)

    def _build(self, m=None):
        self.add_button("Cancel", Gtk.ResponseType.CANCEL)
        save = self.add_button("Save Mount", Gtk.ResponseType.OK)
        _css(save, "suggested-action")
        self.set_default_response(Gtk.ResponseType.OK)

        c = self.get_content_area()
        c.set_spacing(0)
        c.set_margin_start(22)
        c.set_margin_end(22)
        c.set_margin_top(10)
        c.set_margin_bottom(18)

        self.err_lbl = Gtk.Label(xalign=0, wrap=True)
        self.err_lbl.set_no_show_all(True)
        _css(self.err_lbl, "error")
        c.pack_start(self.err_lbl, False, False, 6)

        self._lbl(c, "TYPE")
        self.type_combo = Gtk.ComboBoxText()
        for t in ["NFS", "SMB / CIFS", "SSHFS"]:
            self.type_combo.append_text(t)
        mtype = getattr(m, "mount_type", "nfs") if m else "nfs"
        self.type_combo.set_active(TYPES.index(mtype) if mtype in TYPES else 0)
        self.type_combo.connect("changed", self._type_changed)
        c.pack_start(self.type_combo, False, False, 4)

        self._lbl(c, "DISPLAY NAME")
        self.name_e = self._entry(c, "e.g. Home NAS, Media Server", m, "name")

        self._lbl(c, "HOST")
        self.host_e = self._entry(c, "Hostname or IP address", m, "host")

        self._lbl(c, "REMOTE PATH / SHARE")
        self.remote_e = self._entry(c, "/export/path  or  sharename", m, "remote_path")

        self._lbl(c, "LOCAL PATH")
        lbox = Gtk.Box(spacing=5)
        c.pack_start(lbox, False, False, 4)
        self.local_e = _css(Gtk.Entry(), "mb-entry")
        self.local_e.set_placeholder_text(f"{MOUNTS_DIR}/<name>  (default)")
        self.local_e.set_hexpand(True)
        if m and getattr(m, "local_path", ""):
            self.local_e.set_text(m.local_path)
        lbox.pack_start(self.local_e, True, True, 0)
        bb = Gtk.Button(label="Browse…")
        bb.connect("clicked", self._browse_local)
        lbox.pack_start(bb, False, False, 0)
        self.local_hint = _css(Gtk.Label(xalign=0, wrap=True), "mb-unmanaged-note")
        c.pack_start(self.local_hint, False, False, 0)

        # Credentials (hidden for NFS)
        self.creds_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        c.pack_start(self.creds_box, False, False, 0)
        self._lbl(self.creds_box, "CREDENTIALS")
        grd = Gtk.Grid(column_spacing=8, row_spacing=6)
        self.creds_box.pack_start(grd, False, False, 4)

        self.user_e = _css(Gtk.Entry(), "mb-entry")
        self.user_e.set_placeholder_text("Username (blank = guest)")
        self.user_e.set_hexpand(True)
        if m and getattr(m, "username", None):
            self.user_e.set_text(m.username)
        grd.attach(self.user_e, 0, 0, 1, 1)

        self.pass_e = _css(Gtk.Entry(), "mb-entry")
        self.pass_e.set_placeholder_text(
            "Leave blank to keep saved password" if self.edit else "Password (stored in keyring)")
        self.pass_e.set_visibility(False)
        self.pass_e.set_hexpand(True)
        grd.attach(self.pass_e, 1, 0, 1, 1)

        self.clear_pw = Gtk.CheckButton(label="Forget saved password")
        self.clear_pw.set_no_show_all(not self.edit)
        grd.attach(self.clear_pw, 1, 1, 1, 1)

        # SSH options (SSHFS only)
        self.ssh_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        c.pack_start(self.ssh_box, False, False, 0)
        self._lbl(self.ssh_box, "SSH PORT")
        self.port_e = self._entry(self.ssh_box, "22", m, "port")
        self._lbl(self.ssh_box, "SSH PRIVATE KEY (RECOMMENDED)")
        kbox = Gtk.Box(spacing=5)
        self.ssh_box.pack_start(kbox, False, False, 4)
        self.key_e = _css(Gtk.Entry(), "mb-entry")
        self.key_e.set_placeholder_text("~/.ssh/id_ed25519  (blank = ssh-agent / password)")
        self.key_e.set_hexpand(True)
        if m and getattr(m, "ssh_key", None):
            self.key_e.set_text(m.ssh_key)
        kbox.pack_start(self.key_e, True, True, 0)
        kb = Gtk.Button(label="Browse…")
        kb.connect("clicked", self._browse_key)
        kbox.pack_start(kb, False, False, 0)

        # Windows domain (SMB only)
        self.domain_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        c.pack_start(self.domain_box, False, False, 0)
        self._lbl(self.domain_box, "WINDOWS DOMAIN (OPTIONAL)")
        self.domain_e = self._entry(self.domain_box, "WORKGROUP", m, "domain")

        self._lbl(c, "EXTRA MOUNT OPTIONS (OPTIONAL)")
        self.opts_e = self._entry(c, "Comma-separated, e.g. ro,noatime", m, "options")

        arow = Gtk.Box(spacing=12)
        arow.set_margin_top(12)
        c.pack_start(arow, False, False, 0)
        al = Gtk.Label(label="Auto-mount when MountBridge starts", xalign=0)
        al.set_hexpand(True)
        arow.pack_start(al, True, True, 0)
        self.auto_sw = Gtk.Switch()
        self.auto_sw.set_valign(Gtk.Align.CENTER)
        self.auto_sw.set_active(bool(getattr(m, "auto_mount", False)) if m else False)
        arow.pack_start(self.auto_sw, False, False, 0)

        self.show_all()
        self._type_changed(self.type_combo)

    def _lbl(self, parent, text):
        parent.pack_start(_css(Gtk.Label(label=text, xalign=0), "mb-field-label"),
                          False, False, 0)

    def _entry(self, parent, placeholder, m, attr):
        e = _css(Gtk.Entry(), "mb-entry")
        e.set_placeholder_text(placeholder)
        e.set_activates_default(True)
        val = getattr(m, attr, None) if m else None
        if val not in (None, ""):
            e.set_text(str(val))
        parent.pack_start(e, False, False, 4)
        return e

    def _mtype(self) -> str:
        idx = self.type_combo.get_active()
        return TYPES[idx] if 0 <= idx < len(TYPES) else "nfs"

    def _type_changed(self, _combo):
        t = self._mtype()
        self.creds_box.set_visible(t != "nfs")
        self.ssh_box.set_visible(t == "sshfs")
        self.domain_box.set_visible(t == "smb")
        self.remote_e.set_placeholder_text(
            {"nfs": "/exports/data", "smb": "sharename", "sshfs": "/home/user  or  /"}[t])
        if t == "sshfs":
            self.local_hint.set_text("SSHFS mounts directly at this path.")
        else:
            self.local_hint.set_text(
                f"Mounted at {SYS_MOUNT_BASE}/<name>; this path becomes a shortcut to it.")

    def _browse_local(self, _):
        d = Gtk.FileChooserDialog(title="Choose Local Path", transient_for=self,
                                  action=Gtk.FileChooserAction.SELECT_FOLDER)
        d.add_buttons("Cancel", Gtk.ResponseType.CANCEL, "Select", Gtk.ResponseType.OK)
        MOUNTS_DIR.mkdir(exist_ok=True)
        d.set_current_folder(str(MOUNTS_DIR))
        if d.run() == Gtk.ResponseType.OK:
            self.local_e.set_text(d.get_filename())
        d.destroy()

    def _browse_key(self, _):
        d = Gtk.FileChooserDialog(title="Choose SSH Key", transient_for=self,
                                  action=Gtk.FileChooserAction.OPEN)
        d.add_buttons("Cancel", Gtk.ResponseType.CANCEL, "Open", Gtk.ResponseType.OK)
        d.set_current_folder(str(Path.home() / ".ssh"))
        if d.run() == Gtk.ResponseType.OK:
            self.key_e.set_text(d.get_filename())
        d.destroy()

    def validate(self) -> Optional[str]:
        """Return an error message, or None if the form is valid."""
        v = self.values()
        if not v["name"]:
            return "Enter a display name."
        if not valid_host(v["host"]):
            return "Enter a valid hostname or IP address (IPv6 in [brackets])."
        if v["mount_type"] == "nfs" and not v["remote_path"].startswith("/"):
            return "NFS export path must start with /."
        if v["mount_type"] == "smb" and not v["remote_path"].strip("/"):
            return "Enter the share name."
        lp = v["local_path"]
        if lp and not os.path.isabs(os.path.expanduser(lp)):
            return "Local path must be absolute (or start with ~)."
        ps = self.port_e.get_text().strip()
        if v["mount_type"] == "sshfs" and ps and not (ps.isdigit() and 0 < int(ps) < 65536):
            return "SSH port must be 1–65535."
        if any(ch.isspace() for ch in v["options"]):
            return "Mount options must be comma-separated without spaces."
        return None

    def show_error(self, msg: str):
        self.err_lbl.set_text(msg)
        self.err_lbl.show()

    def values(self) -> dict:
        t = self._mtype()
        ps = self.port_e.get_text().strip()
        creds = t != "nfs"
        return {
            "mount_type":     t,
            "name":           self.name_e.get_text().strip(),
            "host":           self.host_e.get_text().strip(),
            "remote_path":    self.remote_e.get_text().strip(),
            "local_path":     self.local_e.get_text().strip(),
            "username":       (self.user_e.get_text().strip() or None) if creds else None,
            "password":       self.pass_e.get_text() if creds else "",
            "clear_password": self.clear_pw.get_active(),
            "ssh_key":        (self.key_e.get_text().strip() or None) if t == "sshfs" else None,
            "domain":         (self.domain_e.get_text().strip() or None) if t == "smb" else None,
            "port":           int(ps) if t == "sshfs" and ps.isdigit() else None,
            "options":        self.opts_e.get_text().strip(),
            "auto_mount":     self.auto_sw.get_active(),
        }


# ── Managed mount card ───────────────────────────────────────────────────────

class MountCard(Gtk.ListBoxRow):
    def __init__(self, mount: MountConfig, on_toggle: Callable, on_open: Callable,
                 on_edit: Callable, on_delete: Callable):
        super().__init__()
        self.mount = mount
        _css(self, "mb-card")
        self._build(on_toggle, on_open, on_edit, on_delete)

    def _build(self, on_toggle, on_open, on_edit, on_delete):
        m = self.mount
        outer = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
        for side in ("start", "end", "top", "bottom"):
            getattr(outer, f"set_margin_{side}")(14 if side in ("start", "end") else 12)
        self.add(outer)

        top = Gtk.Box(spacing=8)
        top.set_valign(Gtk.Align.CENTER)
        outer.pack_start(top, False, False, 0)
        top.pack_start(_css(Gtk.Label(label=m.mount_type.upper()), "mb-type",
                            f"mb-type-{m.mount_type}"), False, False, 0)

        name_lbl = _css(Gtk.Label(label=m.name, xalign=0), "mb-name")
        name_lbl.set_hexpand(True)
        name_lbl.set_ellipsize(Pango.EllipsizeMode.END)
        top.pack_start(name_lbl, True, True, 0)

        self.dot = _css(Gtk.Box(), "mb-dot", "mb-dot-off")
        top.pack_start(self.dot, False, False, 0)

        if m.mount_type == MountType.SMB:
            remote = f"\\\\{m.host}\\{m.remote_path}"
        elif m.mount_type == MountType.NFS:
            remote = f"{m.host}:{m.remote_path}"
        else:
            remote = f"{m.username + '@' if m.username else ''}{m.host}:{m.remote_path}"
        path_lbl = _css(Gtk.Label(label=f"{remote}  →  {m.local_path}", xalign=0), "mb-remote")
        path_lbl.set_ellipsize(Pango.EllipsizeMode.END)
        path_lbl.set_margin_top(4)
        outer.pack_start(path_lbl, False, False, 0)

        if m.auto_mount:
            al = Gtk.Label(label="⟳ auto-mount", xalign=0)
            al.set_opacity(0.45)
            al.set_margin_top(2)
            outer.pack_start(al, False, False, 0)

        acts = Gtk.Box(spacing=6)
        acts.set_margin_top(10)
        outer.pack_start(acts, False, False, 0)

        self.mount_btn = _css(Gtk.Button(label="Mount"), "mb-btn", "mb-btn-mount")
        self.mount_btn.connect("clicked", lambda _: on_toggle(m))
        acts.pack_start(self.mount_btn, False, False, 0)

        self.open_btn = _css(Gtk.Button(label="Open Folder"), "mb-btn", "mb-btn-secondary")
        self.open_btn.connect("clicked", lambda _: on_open(m))
        self.open_btn.set_sensitive(False)
        acts.pack_start(self.open_btn, False, False, 0)

        spacer = Gtk.Label()
        spacer.set_hexpand(True)
        acts.pack_start(spacer, True, True, 0)

        for icon, tip, cb in (("document-edit-symbolic", "Edit", on_edit),
                              ("user-trash-symbolic", "Remove", on_delete)):
            b = Gtk.Button()
            b.set_image(Gtk.Image.new_from_icon_name(icon, Gtk.IconSize.SMALL_TOOLBAR))
            b.set_tooltip_text(tip)
            b.connect("clicked", lambda _, cb=cb: cb(m))
            acts.pack_start(b, False, False, 0)

        self.show_all()

    def update(self, mounted: bool, busy: bool):
        ctx = self.dot.get_style_context()
        for c in ("mb-dot-on", "mb-dot-off", "mb-dot-busy", "mb-dot-err"):
            ctx.remove_class(c)
        ctx.add_class("mb-dot-busy" if busy else ("mb-dot-on" if mounted else "mb-dot-off"))

        self.open_btn.set_sensitive(mounted and not busy)
        self.mount_btn.set_sensitive(not busy)
        bctx = self.mount_btn.get_style_context()
        if busy:
            self.mount_btn.set_label("Working…")
        elif mounted:
            self.mount_btn.set_label("Unmount")
            bctx.remove_class("mb-btn-mount")
            bctx.add_class("mb-btn-unmount")
        else:
            self.mount_btn.set_label("Mount")
            bctx.remove_class("mb-btn-unmount")
            bctx.add_class("mb-btn-mount")


# ── Section divider ──────────────────────────────────────────────────────────

class SectionRow(Gtk.ListBoxRow):
    """Non-interactive label row separating managed and unmanaged sections."""

    def __init__(self, text: str):
        super().__init__()
        self.set_selectable(False)
        self.set_activatable(False)
        self.add(_css(Gtk.Label(label=text, xalign=0), "mb-section-divider"))
        self.show_all()


# ── Unmanaged mount card ─────────────────────────────────────────────────────

class UnmanagedMountCard(Gtk.ListBoxRow):
    """Card for a live network mount not tracked in the MountBridge config."""

    def __init__(self, lm: LiveMount, on_import: Callable, on_open: Callable,
                 on_unmount: Callable):
        super().__init__()
        self.lm = lm
        _css(self, "mb-card-unmanaged")
        self._build(on_import, on_open, on_unmount)

    def _build(self, on_import, on_open, on_unmount):
        lm = self.lm
        outer = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
        for side in ("start", "end", "top", "bottom"):
            getattr(outer, f"set_margin_{side}")(14 if side in ("start", "end") else 12)
        self.add(outer)

        top = Gtk.Box(spacing=8)
        top.set_valign(Gtk.Align.CENTER)
        outer.pack_start(top, False, False, 0)
        top.pack_start(_css(Gtk.Label(label=lm.mount_type.upper()), "mb-type",
                            f"mb-type-{lm.mount_type}"), False, False, 0)
        top.pack_start(_css(Gtk.Label(label="LIVE"), "mb-type", "mb-type-live"),
                       False, False, 0)
        name_lbl = _css(Gtk.Label(label=f"{lm.host} — {lm.remote_path}", xalign=0), "mb-name")
        name_lbl.set_hexpand(True)
        name_lbl.set_ellipsize(Pango.EllipsizeMode.END)
        top.pack_start(name_lbl, True, True, 0)
        top.pack_start(_css(Gtk.Box(), "mb-dot", "mb-dot-on"), False, False, 0)

        path_lbl = _css(Gtk.Label(label=f"{lm.device}  →  {lm.local_path}", xalign=0),
                        "mb-remote")
        path_lbl.set_ellipsize(Pango.EllipsizeMode.END)
        path_lbl.set_margin_top(4)
        outer.pack_start(path_lbl, False, False, 0)
        outer.pack_start(_css(Gtk.Label(
            label="Not managed by MountBridge — import to track, edit and auto-mount it.",
            xalign=0), "mb-unmanaged-note"), False, False, 0)

        acts = Gtk.Box(spacing=6)
        acts.set_margin_top(10)
        outer.pack_start(acts, False, False, 0)

        imp = _css(Gtk.Button(label="Import to Config"), "mb-btn", "mb-btn-mount")
        imp.connect("clicked", lambda _: on_import(lm))
        acts.pack_start(imp, False, False, 0)
        opn = _css(Gtk.Button(label="Open Folder"), "mb-btn", "mb-btn-secondary")
        opn.connect("clicked", lambda _: on_open(lm.local_path))
        acts.pack_start(opn, False, False, 0)
        spacer = Gtk.Label()
        spacer.set_hexpand(True)
        acts.pack_start(spacer, True, True, 0)
        self.unmount_btn = _css(Gtk.Button(label="Unmount"), "mb-btn", "mb-btn-unmount")
        self.unmount_btn.connect("clicked", lambda _: on_unmount(self))
        acts.pack_start(self.unmount_btn, False, False, 0)
        self.show_all()

    def set_busy(self, busy: bool):
        self.unmount_btn.set_label("Working…" if busy else "Unmount")
        self.unmount_btn.set_sensitive(not busy)


# ── Network discovery panel ──────────────────────────────────────────────────

class DiscoveryPanel(Gtk.Box):
    def __init__(self, disc: Discovery, on_add: Callable):
        super().__init__(orientation=Gtk.Orientation.VERTICAL, spacing=0)
        self.disc = disc
        self.on_add = on_add
        self._build()

    def _build(self):
        tb = Gtk.Box(spacing=8)
        tb.set_margin_start(12)
        tb.set_margin_end(12)
        tb.set_margin_top(14)
        tb.set_margin_bottom(10)
        self.pack_start(tb, False, False, 0)

        self.mode_combo = Gtk.ComboBoxText()
        for t in ["SMB broadcast", "SMB shares on host", "NFS exports on host"]:
            self.mode_combo.append_text(t)
        self.mode_combo.set_active(0)
        self.mode_combo.connect("changed", self._mode_changed)
        tb.pack_start(self.mode_combo, False, False, 0)

        self.host_e = Gtk.Entry()
        self.host_e.set_placeholder_text("Target host (for SMB shares / NFS)")
        self.host_e.set_hexpand(True)
        self.host_e.set_no_show_all(True)
        self.host_e.connect("activate", self._scan)
        tb.pack_start(self.host_e, True, True, 0)

        self.scan_btn = _css(Gtk.Button(label="Scan Network"), "suggested-action")
        self.scan_btn.connect("clicked", self._scan)
        tb.pack_start(self.scan_btn, False, False, 0)

        self.spinner = Gtk.Spinner()
        tb.pack_start(self.spinner, False, False, 0)
        self.pack_start(Gtk.Separator(), False, False, 0)

        scroll = Gtk.ScrolledWindow()
        scroll.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        scroll.set_vexpand(True)
        self.pack_start(scroll, True, True, 0)
        self.lb = Gtk.ListBox()
        self.lb.set_selection_mode(Gtk.SelectionMode.NONE)
        scroll.add(self.lb)

        self._add_hint("Use the scanner above to find NFS and SMB shares on your network.")
        self.show_all()

    def _mode_changed(self, combo):
        self.host_e.set_visible(combo.get_active() in (1, 2))

    def _clear(self):
        for r in self.lb.get_children():
            self.lb.remove(r)

    def _add_hint(self, text):
        row = Gtk.ListBoxRow()
        row.set_selectable(False)
        lbl = Gtk.Label(label=text, xalign=0, wrap=True)
        lbl.set_opacity(0.45)
        lbl.set_margin_start(14)
        lbl.set_margin_top(16)
        row.add(lbl)
        self.lb.add(row)
        self.lb.show_all()

    def _busy(self, busy: bool):
        self.scan_btn.set_sensitive(not busy)
        (self.spinner.start if busy else self.spinner.stop)()

    def _scan(self, _):
        self._clear()
        mode = self.mode_combo.get_active()
        host = self.host_e.get_text().strip()
        if mode in (1, 2) and not valid_host(host):
            self._add_hint("Enter a valid hostname or IP address first.")
            return
        self._busy(True)
        if mode == 0:
            self.disc.smb_broadcast(self._on_broadcast)
        elif mode == 1:
            self.disc.smb_shares(host, "", "", self._on_smb_shares)
        else:
            self.disc.nfs_exports(host, self._on_nfs_exports)

    def _on_broadcast(self, results):
        self._busy(False)
        if not results:
            self._add_hint("No SMB hosts found via Avahi. Try specifying a host directly.")
            return
        for r in results:
            self._result_row(r["host"], None, "smb", r.get("detail", ""))
        self.lb.show_all()

    def _on_smb_shares(self, host, shares):
        self._busy(False)
        if not shares:
            self._add_hint(f"No shares found on {host} (anonymous listing may be disabled).")
            return
        for s in shares:
            self._result_row(host, s, "smb", f"\\\\{host}\\{s}")
        self.lb.show_all()

    def _on_nfs_exports(self, host, exports):
        self._busy(False)
        if not exports:
            self._add_hint(f"No NFS exports found on {host}. "
                           "NFSv4-only servers don't answer showmount — add the mount manually.")
            return
        for p in exports:
            self._result_row(host, p, "nfs", f"{host}:{p}")
        self.lb.show_all()

    def _result_row(self, host, path, mtype, detail):
        row = Gtk.ListBoxRow()
        box = _css(Gtk.Box(spacing=12), "mb-disc-row")
        row.add(box)
        icons = {"smb": "network-workgroup-symbolic", "nfs": "drive-harddisk-symbolic"}
        box.pack_start(Gtk.Image.new_from_icon_name(
            icons.get(mtype, "network-server-symbolic"), Gtk.IconSize.BUTTON), False, False, 0)

        info = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        info.set_hexpand(True)
        box.pack_start(info, True, True, 0)
        info.pack_start(_css(Gtk.Label(label=host, xalign=0), "mb-disc-host"), False, False, 0)
        if detail:
            info.pack_start(_css(Gtk.Label(label=detail, xalign=0), "mb-disc-detail"),
                            False, False, 0)

        box.pack_start(_css(Gtk.Label(label=mtype.upper()), "mb-type", f"mb-type-{mtype}"),
                       False, False, 0)
        add_btn = Gtk.Button(label="Add Mount")
        add_btn.connect("clicked", lambda _, h=host, p=path, t=mtype: self.on_add(h, p, t))
        box.pack_start(add_btn, False, False, 0)
        self.lb.add(row)


def default_local_path(name: str, taken: set) -> str:
    """~/.mounts/<slug>, suffixed -2, -3… if another mount already uses it."""
    base = slugify(name)
    candidate, n = base, 2
    while str(MOUNTS_DIR / candidate) in taken:
        candidate, n = f"{base}-{n}", n + 1
    return str(MOUNTS_DIR / candidate)
